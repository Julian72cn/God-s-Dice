"""
MC_simulation_1.3.py

This script runs multiple stock market model simulations (based on main_OHLC_2.0.5.1.py),
simulating different capital injection scenarios in the market, and analyzes the impact of capital flows
on stock price averages using Monte Carlo methods.

Simulating three scenarios:
1. Baseline: No capital injection
2. One-time injection: Injecting 50% of each investor's cash at day 200
3. Batch injection: Injecting 10% of each investor's cash at 5 different time points, 50 days apart

Main features:
1. Simulating market performance under different capital injection strategies
2. Comparing the effects of one-time large-scale injection versus batch small-scale injections
3. Reducing the influence of random factors through multiple simulations to obtain more reliable conclusions

This simulation helps understand the mechanism of different capital injection strategies on market prices, providing reference for investment decisions.
"""
...
def plot_capital_change_comparison(results, capital_change_scenarios):
    ...
    for scenario in results:
        ...
        for sequence in all_sequences:
            ax.plot(sequence[:min_length], alpha=0.8, color='darkgray', linewidth=0.8, zorder=2)
    ...
    # Hide extra subplots if number of scenarios is less than rows*cols
...
def main():
    ...
    num_simulations = 50  # Number of simulations for each capital change strategy

    # Define different capital change strategies
    capital_change_scenarios = {
        'baseline': None,  # Baseline case, no capital change

        'one_time_injection_50pct': [
            # One-time injection of 50% of each investor's cash
            {'day': 200, 'type': 'inject', 'percentage': 0.5}
        ],

        'five_times_injection_10pct': [
            # Five injections of 10% of each investor's cash
            {'day': 100, 'type': 'inject', 'percentage': 0.1},
            {'day': 150, 'type': 'inject', 'percentage': 0.1},
            {'day': 200, 'type': 'inject', 'percentage': 0.1},
            {'day': 250, 'type': 'inject', 'percentage': 0.1},
            {'day': 300, 'type': 'inject', 'percentage': 0.1}
        ]
    }
