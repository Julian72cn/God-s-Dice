"""
butterfly_effect_simulation_1.4.py

This script simulates the butterfly effect in the stock market by introducing 
a sudden investor who starts trading on a specific date, then observes how 
this small change affects the subsequent market behavior.

Changes:
1. Modified based on previous version
2. Reduced the number of investors (except SuddenInvestor) to half of the original number (50x compared to initial version)
3. Observe whether the sudden investor's impact on the market is enhanced in a medium-sized market
4. Analyze the non-linear relationship between market size and butterfly effect intensity
...
def run_baseline_simulation(random_seeds, days=500, enable_trade_log=False):
    """
    Run baseline simulation (without closing price modification).
    ...
    # Different investor type parameters (numbers increased 50x)
    value_investors_params = {
        'num': 5000,  # Originally 100, increased 50x
        'initial_shares': 100,
        'initial_cash': 10000
    }

    chase_investors_params = {
        'num': 2500,  # Originally 50, increased 50x
        'initial_shares': 100,
        'initial_cash': 10000
    }
    ...
    random_investors_params = {
        'num': 1500,  # Originally 30, increased 50x
        'initial_shares': 100,
        'initial_cash': 10000
    }

    never_stop_loss_investors_params = {
        'num': 1000,  # Originally 20, increased 50x
        'initial_shares': 100,
        'initial_cash': 10000
    }
    ...
    insider_investors_params = {
        'num': 250,  # Originally 5, increased 50x
        'initial_shares': 100,
        'initial_cash': 10000
    }

    message_investors_params = {
        'num': 500,  # Originally 10, increased 50x
        'initial_shares': 100,
        'initial_cash': 10000
    }
    ...
    # Run simulation trading day by day
...
def run_modified_simulation(random_seeds, action_day, investor_cash, days=500, enable_trade_log=False):
    """
    Run modified simulation with a sudden investor added on a specified date.

    Parameters:
        random_seeds: Dictionary containing various random seeds to control simulation randomness
        action_day: Trading day when the sudden investor executes buy operation (counting from 0)
        investor_cash: Initial funds for the sudden investor
        days: Number of trading days to simulate
        enable_trade_log: Whether to enable trade logging, defaults to False
    ...
    # Add sudden investor who participates in trading
    # Set price premium
    price_premium = 0.05  # Price premium, 5%, increase premium to make orders more likely to execute
    sudden_investor = SuddenInvestor(
        shares=0,  # Initially holds no stocks
        cash=investor_cash,  # Set initial funds
    ...
    # Add this investor separately, not to the regular investor list, to ensure it only participates on the specified date
    ...
    for day in range(days):
        ...
        # Let sudden investor participate in trading on the specified date
        if day == action_day:
            ...
            # Let sudden investor participate in trading
            sudden_investor.trade(market.price, market)

        # Execute daily auction
        market.daily_auction()

        # Print market conditions and check trade execution status on the specified date
        if day == action_day:
            # Print the day's market price and trading volume
        ...
        # Check trade status again one day after the action day
        if day == action_day + 1:
...
def calculate_price_differences(baseline_prices, modified_prices):
    """
    Calculate the difference between baseline prices and modified prices.
    ...
    Returns:
        Percentage sequence of price differences
    """
...
def plot_butterfly_effect(baseline_market, modified_markets, action_day, investor_cash_amounts):
    """
    Plot visualization charts of the butterfly effect.

    Parameters:
        baseline_market: Market object from baseline simulation
        modified_markets: List of market objects from modified simulations
        action_day: Trading day when the sudden investor executes buy operation
        investor_cash_amounts: List of initial funds for sudden investors
    """
    ...
    # Plot value curve (placed at the bottom layer)
    ax1.plot(value_curve, label='True Value', color='dimgray', linestyle='--', linewidth=1.5, alpha=0.7, zorder=1)

    # Use unified color scheme, not distinguishing between positive and negative changes
    ...
    for i, (market, cash) in enumerate(zip(modified_markets, investor_cash_amounts)):
        ...
        # 根据变化幅度选择颜色和标签
        color = all_colors[i % len(all_colors)]  # Use cycling method to select colors
        # Use fund amount as label
        label = f'Cash {cash:.0f}'
        alpha = 0.8

        # All modified lines use unified line style and width, but thinner than baseline
        ax1.plot(modified_prices, label=label, color=color, alpha=alpha, linestyle='-',
                 linewidth=0.8, zorder=2+i)

    # Add vertical line to mark investor action date
    ...
    # Plot baseline line (placed at the top layer)
    ax1.plot(baseline_prices, label='Baseline', color='black', linewidth=1.2, zorder=20)

    ax1.set_title('Butterfly Effect of Sudden Investor in Medium Market (50x Investors)')
    ax1.set_xlabel('Trading Days')
    ax1.set_ylabel('Price')
    # Use more compact legend layout, displayed in two columns
    ...
    for i, (market, cash) in enumerate(zip(modified_markets, investor_cash_amounts)):
        ...
        # All lines use unified line style and width
    ...
    # Save figure first, then display
    print("Saving figure to 'butterfly_effect_sudden_investor_medium_market.png'...")
    plt.savefig('butterfly_effect_sudden_investor_medium_market.png', dpi=600)
    print("Figure saved successfully.")

    # Save a low-resolution version for quick viewing
    plt.savefig('butterfly_effect_sudden_investor_medium_market_low_res.png', dpi=100)
...
def analyze_butterfly_effect(baseline_market, modified_markets, investor_cash_amounts):
    """
    Analyze the impact level of the butterfly effect.
    ...
    Returns:
        Analysis results list, each element is a dictionary containing analysis data
    """
    baseline_prices = [ohlc[3] for ohlc in baseline_market.ohlc_data]
    ...
    for market, cash in zip(modified_markets, investor_cash_amounts):
        ...
        # Calculate impact ratio (maximum difference to investor funds ratio, unit: percentage/10,000 yuan)
        impact_ratio = max_diff / (cash / 10000) if cash > 0 else 0

        # Determine the direction of final difference
        if final_diff > 0:
            direction = "Positive"
        elif final_diff < 0:
            direction = "Negative"
...
def save_simulation_data(market, filename):
    """Save simulation data to file"""
    try:
        # Extract closing prices
        prices = [ohlc[3] for ohlc in market.ohlc_data]

        # Create directory for saving data
        os.makedirs('simulation_data', exist_ok=True)

        # Save data to CSV file
        with open(f'simulation_data/{filename}.csv', 'w') as f:
            f.write('Day,Price\n')
            for i, price in enumerate(prices):
                f.write(f'{i},{price}\n')

        print(f"Simulation data saved to 'simulation_data/{filename}.csv'")
    except Exception as e:
        print(f"Error saving simulation data: {e}")

def main():
    # Set global random seed to make results reproducible
    global_seed = 44
    ...
    days = 1000  # Number of trading days to simulate, reduced to speed up simulation
    action_day = 300  # Trading day when the sudden investor executes buy operation
    investor_cash_amounts = [300, 1000, 5000, 10000, 20000, 50000, 100000]  # List of initial funds for sudden investors
    ...
    save_simulation_data(baseline_market, "baseline_simulation")
    ...
    for cash in investor_cash_amounts:
        ...
        # Save each modified simulation data
        save_simulation_data(modified_market, f"modified_simulation_cash_{cash}")

    # Analyze butterfly effect
    analysis_results = analyze_butterfly_effect(baseline_market, modified_markets, investor_cash_amounts)
    ...
    try:
        ...
        with open('simulation_data/butterfly_effect_analysis.txt', 'w') as f:
            f.write("Butterfly Effect Analysis Results:\n")
            f.write("-" * 80 + "\n")
            f.write(f"{'Cash':^10}{'Max Diff':^15}{'Final Diff':^15}{'Avg Diff':^15}{'Impact Ratio':^15}{'Direction':^10}\n")
            f.write("-" * 80 + "\n")
            for result in analysis_results:
                f.write(f"{result['cash']:^10.0f}{result['max_diff']:^15.2f}%{result['final_diff']:^15.2f}%{result['avg_diff']:^15.2f}%{result['impact_ratio']:^15.2f}{result['direction']:^10}\n")
            f.write("-" * 80 + "\n")
        print("Analysis results saved to 'simulation_data/butterfly_effect_analysis.txt'")
    except Exception as e:
    ...
    print("\nSimulation completed. Results saved to 'butterfly_effect_sudden_investor_medium_market.png'")
