"""
butterfly_effect_simulation_1.3.py
...
Changes:
1. Modified based on butterfly_effect_simulation_1.2.py
2. Increased the number of all investors (except SuddenInvestor) by 10 times
3. Observe whether the impact of sudden investor on the market changes with larger market size
4. Analyze the relationship between market size and butterfly effect intensity
...
def run_baseline_simulation(random_seeds, days=2000, enable_trade_log=False):
    ...
    # Different investor type parameters (numbers increased 10x)
    value_investors_params = {
        'num': 1000,  # Was 100, increased 10x
        'initial_shares': 100,
        'initial_cash': 10000
    }

    chase_investors_params = {
        'num': 500,  # Was 50, increased 10x
        'initial_shares': 100,
        'initial_cash': 10000
    }
    ...
    random_investors_params = {
        'num': 300,  # Was 30, increased 10x
        'initial_shares': 100,
        'initial_cash': 10000
    }

    never_stop_loss_investors_params = {
        'num': 200,  # Was 20, increased 10x
        'initial_shares': 100,
        'initial_cash': 10000
    }
    ...
    insider_investors_params = {
        'num': 50,  # Was 5, increased 10x
        'initial_shares': 100,
        'initial_cash': 10000
    }

    message_investors_params = {
        'num': 100,  # Was 10, increased 10x
        'initial_shares': 100,
        'initial_cash': 10000
    }
    ...
    bias_percent_std = 0.30  # Was 0.15
    ...
    # Create chase investors (chasing price uptrend)
    ...
    # Create never stop loss investors (no stop loss strategy)
    ...
    for i in range(bottom_fishing_investors_params['num']):
        # Use random number generator to create profit target
    ...
    # Calculate start and end indices for each investor type (for later analysis)
...
def run_modified_simulation(random_seeds, action_day, investor_cash, days=1000, enable_trade_log=False):
    """
    Run modified simulation by adding a sudden investor on a specific date.
...
def plot_butterfly_effect(baseline_market, modified_markets, action_day, investor_cash_amounts):
    ...
    # Use unified color scheme, no distinction between positive and negative changes
    ...
    for i, (market, cash) in enumerate(zip(modified_markets, investor_cash_amounts)):
        ...
        # All modified lines use uniform linestyle and width, but thinner than baseline
    ...
    ax1.set_title('Butterfly Effect of Sudden Investor in Large Market (10x Investors)')
    ax1.set_xlabel('Trading Days')
    ax1.set_ylabel('Price')
    # Use more compact legend layout, display in two columns
    ...
    for i, (market, cash) in enumerate(zip(modified_markets, investor_cash_amounts)):
        ...
        # All lines use uniform linestyle and width
    ...
    plt.savefig('butterfly_effect_sudden_investor_large_market.png', dpi=600)
    plt.show()
...
def analyze_butterfly_effect(baseline_market, modified_markets, investor_cash_amounts):
    ...
    for market, cash in zip(modified_markets, investor_cash_amounts):
        ...
        # Calculate impact ratio (max diff to investor cash ratio, in %/10k)
        impact_ratio = max_diff / (cash / 10000) if cash > 0 else 0

        # Determine direction of final difference
        if final_diff > 0:
            direction = "Positive"
        elif final_diff < 0:
            direction = "Negative"
...
def main():
    ...
    print("\nSimulation completed. Results saved to 'butterfly_effect_sudden_investor_large_market.png'")
